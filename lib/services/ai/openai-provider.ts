/**
 * OpenAI AI provider (FUTURE OPTION).
 *
 * Production note:
 * - When enabling OpenAI:
 *     1. Add OPENAI_API_KEY (see config/env.ts).
 *     2. Set DEFAULT_AI_PROVIDER = 'openai' in config/app.ts (or select per
 *        request) and use the AI SDK with model `${AI_MODELS.openai}`.
 *     3. Replace placeholder returns with real `generateObject`/`generateText`
 *        calls, validating output with the Zod schemas in lib/validations.
...
export class OpenAIProvider implements AIProvider {
  ...
  private readonly model = AI_MODELS.openai

  async generateSummary(req: SummaryRequest): Promise<Summary> {
    // TODO(openai): call generateText with `${this.model}`.
    return {
      id: `sum_${req.documentId}`,
      documentId: req.documentId,
      subjectId: req.subjectId ?? '',
      content: req.content ?? 'Summary will be generated by OpenAI.',
